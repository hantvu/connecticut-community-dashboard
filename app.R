...
ui <- dashboardPage(
  dashboardHeader(
    ...
    title = "Connecticut Community Dashboard"
  ),
  dashboardSidebar(
    # Add narrative text to the sidebar with working links
    tags$div(
      style = "padding: 10px; font-family: sans-serif; font-size: 14px;",
      HTML("<h4>Welcome to the Connecticut Community Dashboard</h4>
           <p>This dashboard provides an overview of various demographic and socioeconomic factors for different counties and areas in Connecticut. Explore the data and maps to gain insights into the community.</p>
...
server <- function(input, output, session) {
  ...
  income_data <- get_acs(
    ...
    mutate(NAME = str_remove(NAME, " County, Connecticut"))
  ...
  output$income_chart <- renderPlot({
    ggplot(income_data, aes(x = estimate, y=reorder(NAME, estimate))) +
      ...
      labs(title = "Median Household Income",
           subtitle = "Counties in Connecticut",
  ...
  # ct_filtered <- filter(ct_pop, str_detect(AGEGROUP, "^Age"), 
  #                       SEX != "Both sexes") %>%
  ...
  output$gender_age_chart <- renderPlot({
    ggplot(pyramid_data, aes(x = pyramid_value, y = variable,
      ...
      labs(x = "",
           y = "",
           title = "Population structure in Connecticut",
           fill = "",
  ...
  output$race_map <- renderLeaflet({
    leaflet(options = leafletOptions(zoomControl = FALSE)) %>%
      ...
      setView(lng = -72.685093, lat = 41.6032, zoom = 8.5)
  })
  ...
  output$line_chart <- renderPlot({
    ggplot(all_years, aes(x = as.numeric(variable), y = value, color = year, group = year)) +
      ...
      labs(y = "Population",
           x = "Single-year age",
           color = "Year",
           title = glue::glue("Age distribution in Connecticut")) +
